...
uint32_t SHA1::ff(size_t t, uint32_t B, uint32_t C, uint32_t D) {
	if (t >= 0 && t <= 19) {
		return (B & C) | ((~B) & D);
	}
...
void SHA1::final_pad_message(std::vector<uint8_t> message, size_t l, uint32_t& H0, uint32_t& H1, uint32_t& H2, uint32_t& H3, uint32_t& H4) {
...
#endif
		SHA1::process_block(message, H0, H1, H2, H3, H4);
...
uint32_t SHA1::circular_left_shift(size_t n, uint32_t X) {
	return (X << n) | (X >> (32 - n));
}

void SHA1::process_block(const std::vector<uint8_t>& buffer_block, uint32_t& H0, uint32_t& H1, uint32_t& H2, uint32_t& H3, uint32_t& H4) {
	/* a. */
...
std::string SHA1::hash(const std::string& file_path) {
...
bool SHA1::compare_files(const std::string& file1_path, const std::string& file2_path) {
	if (file1_path == file2_path) {
		return true;
	}

	std::future<std::string> hash1 = std::async(SHA1::hash, file1_path);
	std::future<std::string> hash2 = std::async(SHA1::hash, file2_path);
